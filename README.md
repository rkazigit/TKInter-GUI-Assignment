...
| 3. **Combobox dropdowns** | change either the From or To dropdown while a number is typed (the result updates immediately). |
| 4. **Swap button** | click Swap to reverse the units and recalculate (km/h → mph becomes mph → km/h). |
| 5. **Decimal places spinner** | click the up/down arrows in the toolbar to change precision (0-10 decimal places). |
| 6. **Error: non-numeric input** | type `hello` → red error message appears. |
...
| 10. **Clear All** | click Clear All in the toolbar (or press Ctrl+L) → every tab's input and result resets at once. |
| 11. **Hover effects** | hover over the Convert, Swap, and Clear All buttons. Each one changes colour. Move away and it returns to normal. |
| 12. **Tab icons** | each tab header shows a different coloured icon (you need to install Pillow for this). |
| 13. **Help How to Use** | click Help in the menu bar → How to Use popup opens with instructions, closes with the Close button. |
| 14. **Settings Decimal Precision** | same precision control as the toolbar spinner, but accessible through the menu bar. |
